    ...
    <>
      <Modal isOpen={isOpen} onOpenChange={onOpenChange}>
        <ModalContent>
          {() => (
            <>
              <ModalHeader className="flex flex-col gap-1">Price History for {family}</ModalHeader>
              <ModalBody>
      ...
      <Table isHeaderSticky aria-label="Price Table" className="max-h-96 overflow-y-auto w-full">
      <TableHeader>
        ...
        {(item) => (
          <TableRow key={item.name}>
            ...
            <TableCell>
              <Tooltip content={
                <div>
                  {item.commodities.map((commodity: { name: any; price: any; }, index: number) => (<Fragment key={index}>{commodity.name}: {commodity.price}<br /></Fragment>))}
                </div>
              } showArrow={true}>
                <div className="flex flex-row items-center gap-3 w-full">
                <p>{item.average}</p>
            ...
            <TableCell>
              <p className={item.changeSign === -1 ? "text-green-300" : item.changeSign === 1 ? "text-red-300" : ""}>
                {item.changePercent}
              </p>
            </TableCell>
            <TableCell>
