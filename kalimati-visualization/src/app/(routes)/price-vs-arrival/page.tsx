...
const trendlineColor = "rgba(255, 255, 255, 0.6)";
...
export default function PriceArrivalScatter() {
  ...
  function onLineToggle(enabled: boolean) {
    ...
    if (!chartRef.current) return;
    // @ts-ignore
    chartRef.current.data.datasets[0].trendlineLinear = enabled ? trendlineConfig : undefined;
    chartRef.current.update();
  }
  ...
  return (
    <div className="flex flex-col p-4 justify-center items-center w-full h-screen">
      <div className="pb-20 flex gap-10 flex-row">
        <Autocomplete className="max-w-xs" defaultItems={families} label="Commodity" placeholder="Select a commodity" onSelectionChange={onSelect}>
          {(f) => <AutocompleteItem>{f.label}</AutocompleteItem>}
        </Autocomplete>
        <Switch isSelected={lineEnabled} onValueChange={onLineToggle}>Trendline</Switch>
      </div>
